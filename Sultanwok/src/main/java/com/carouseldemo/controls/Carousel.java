...
import com.app2mobile.utiles.CircleTransform;
...
//	private ImageLoader imageLoaderobj;
	 ...
	 * Max theta
	 */
	private static final float MAX_THETA = 0.0f;
...
//		imageLoaderobj = new ImageLoader(context);
	 ...
	 * 
	 */
	public void initilize(ArrayList<RestaurantCategoryMetadata> categorylist,
			int selectedItem) {
		// Initialize image adapter
		System.out.println("categoryList " + categorylist.size());
		ArrayList<RestaurantCategoryMetadata> dataList = new ArrayList<RestaurantCategoryMetadata>();
		ImageAdapter adapter = new ImageAdapter(getContext());
		dataList.addAll(categorylist);
		Collections.reverse(dataList);
		adapter.SetImages(dataList, mUseReflection);
		setAdapter(adapter);
	...
	private class ImageAdapter extends BaseAdapter {
		...
		public void SetImages(ArrayList<RestaurantCategoryMetadata> array,
				boolean reflected) {
			...
			for (int i = 0; i < array.size(); i++) {
				// Bitmap originalImage = imageLoaderobj.getBitmap(array.get(i)
				// .getImage());
				...
				// (int) getResources().getDimension(R.dimen.carousel),
				// (int) getResources().getDimension(R.dimen.carousel),
				// true);
				//
				// if (originalImage != null) {
				...
				// // Set the Transfer mode to be porter duff and
				// // destination in
				...
				// imageLoaderobj.DisplayImage(array.get(i).getCatImg(),
				// item.mImage, 1);
				Picasso.with(mContext).load(array.get(i).getCatImg())
				...
				// R.dimen.category_height_grid))
						...
						.placeholder(R.drawable.ic_launcher).into(item.mImage);
				// if (names != null)
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (mImages.get(position).getCurrentAngle() < 270
					&& mImages.get(position).getCurrentAngle() > 90) {
	...
	public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX,
		...
		// Track the motion
		trackMotionScroll(/* -1 * */(int) (distanceX * .1));
	...
	private void Calculate3DPosition(CarouselItem child, int diameter,
		...
		float x = -(float) (diameter / 2 * (float)Math
				.sin(angleOffset)) + diameter / 2 - child.getWidth() / 2;
		float z = diameter / 2
				* (1.0f - (float) (float)Math.cos(angleOffset));
		float y = -getHeight() / 2
				+ (float) (z * (float)Math.sin(mTheta));

		child.setItemX(x);
		// child.setItemZ((1.0f - (float)
		// android.util.FloatMath.cos(angleOffset))
		// * child.getWidth());
		child.setItemZ(z);
		child.setItemY(y + getHeight() / 2);
